...
def aml_words_JURISDICTION_BANK():
    rowValues = list(jurisdictionBankList.objects.values('bank_name'))
    ...
    for i in rowValues:
        print(i)
        sns_text += i['bank_name'].replace('\ufeff', ' ').replace('\n', '').replace('#', ' ').replace('_', ' ').replace('!', '').replace(
